...
// TestMySQLDBWithOpts sets up the test DB to use, including running any
// migrations. In case the ReuseDB option is set to true, this function might
// not create a new database.
//
// This expects a local MySQL to be running with default "root/root"
...
//func TestMySQLDBWithOpts(t testing.T, dbName string, opts *TestDBOptions) *gorm.DB {
//	t.Helper()
//
...
//	// drop it.
//	db := testDBConnectWithUser(t, FamilyMySQL, "", "root", "root")
//	if *gormDebug {
//		db.LogMode(true)
//	}
//
...
//	// latest version.
//	if !opts.ReuseDB || !mysqlDBInitialized {
//		db.Exec("DROP DATABASE IF EXISTS " + dbName + ";")
//		db.Exec("CREATE DATABASE " + dbName + ";")
//		db.Exec(fmt.Sprintf("CREATE USER IF NOT EXISTS %s IDENTIFIED BY '%s';", UserName, UserPassword))
//		db.Close()
//
//		if !opts.SkipMigration {
//			// Migrate using our migrations
//			testMigrate(t, FamilyMySQL, dbName)
//		}
//
...
//		// connection since the old one was not connected to our test db we
//		// just created so the grant is applied to the mySQL db instead.
//		db = TestDBConnectSuper(t, FamilyMySQL, dbName)
//		db.Exec("GRANT ALL PRIVILEGES ON *.* TO " + UserName + ";")
//
//		db.Close()
//		mysqlDBInitialized = true
//	} else {
...
//			Pluck("TABLE_NAME", &tablesToTruncate)
//		if errs := db.GetErrors(); len(errs) > 0 {
//			t.Errorf("unable to determine tablesToTruncate to truncate: %v",
...
//		if errs := db.GetErrors(); len(errs) > 0 {
//			t.Errorf("failed to disable foreign key checks: %v",
//				multierror.Append(nil, errs...),
//			)
//		}
//
//		for _, table := range tablesToTruncate {
//			// Do not truncate tables that are to be kept.
//			if _, ok := tablesToKeep[table]; ok {
//				continue
//			}
//			db = db.Exec(fmt.Sprintf("TRUNCATE TABLE %s.%s;", dbName, table))
//			if errs := db.GetErrors(); len(errs) > 0 {
//				t.Errorf("failed to truncate table %q: %v",
//					table,
//					multierror.Append(nil, errs...),
//				)
//			}
...
//		if errs := db.GetErrors(); len(errs) > 0 {
//			t.Errorf("failed to enable foreign key checks: %v",
//				multierror.Append(nil, errs...),
//			)
//		}
//		db.Close()
//	}
//
//	return TestDBConnect(t, FamilyMySQL, dbName)
//}

// TestMySQLDB sets up the test DB to use, including running any migrations.
//
// This expects a local MySQL to be running with default "root/root"
// superuser credentials.
//
...
// path represented by MigrationsDir where the migration files can be found.
// If no migrations are found, then an error is raised.
//func TestMySQLDB(t testing.T, dbName string) *gorm.DB {
//	return TestMySQLDBWithOpts(t, dbName, &TestDBOptions{})
//}
