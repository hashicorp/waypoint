...
func RunnerGetDeploymentConfig(
	s Service,
	ctx context.Context,
...
func RunnerToken(
	...
	if tok := tokenFromContext(s, ctx); tok != nil {
		...
		case *pb.Token_Runner_:
			if k.Runner.Id != "" {
				...
				if err != nil {
					log.Error("Failed to parse id", "id", k.Runner.Id, "err", err)
...
func RunnerConfig(
	...
	if err := runnerVerifyToken(s, log, ctx, record.Id, record.Labels); err != nil {
		return err
	}
	...
	log = log.With("runner_id", record.Id)
	log.Trace("registering runner")
	if err := state.RunnerCreate(record); err != nil {
		return err
	}
	...
	defer func() {
		log.Trace("marking runner as offline")
		if err := state.RunnerOffline(record.Id); err != nil {
			log.Error("failed to mark runner as offline. This should not happen.", "err", err)
		}
	}()
	...
	r, err := state.RunnerById(record.Id, nil)
	if err != nil {
		return err
	}
	...
	if r.AdoptionState != pb.Runner_ADOPTED {
		if err := state.RunnerAdopt(record.Id, true); err != nil {
			return err
		}
	}
	...
	if _, ok := record.Kind.(*pb.Runner_Odr); ok {
		...
		sjob, err := state.JobPeekForRunner(ctx, record)
		if err != nil {
			return err
		}
		if sjob == nil {
	...
	for {
		...
		vars, err := state.ConfigGetWatch(configReq, ws)
		if err != nil {
			return err
		}
		...
		if varContainsDynamic(vars) || !isLocal {
			...
			sources, err := state.ConfigSourceGetWatch(&pb.GetConfigSourceRequest{
...
func RunnerJobStream(
	...
	runner, err := state.RunnerById(reqEvent.Request.RunnerId, nil)
	if err != nil {
	...
	job, err := state.JobAssignForRunner(ctx, runner)
	if err != nil {
		return err
	}
	...
	job, ackerr := state.JobAck(job.Id, ack)
	if ackerr != nil {
	...
	go func() {
		for {
			ws := memdb.NewWatchSet()
			job, err = state.JobById(job.Id, ws)
			if err != nil {
				errCh <- err
				return
			}
	...
	for {
		...
		case <-ctx.Done():
			...
			for {
				select {
				case req := <-eventCh:
					if err := handleJobStreamRequest(state, log, job, server, req); err != nil {
						return err
					}
				default:
					return nil
...
func handleJobStreamRequest(
	...
	switch event := req.Event.(type) {
	case *pb.RunnerJobStreamRequest_Complete_:
		return state.JobComplete(job.Id, event.Complete.Result, nil)

	case *pb.RunnerJobStreamRequest_Error_:
		return state.JobComplete(job.Id, nil, status.FromProto(event.Error.Error).Err())

	case *pb.RunnerJobStreamRequest_Heartbeat_:
		return state.JobHeartbeat(job.Id)

	case *pb.RunnerJobStreamRequest_Download:
		if err := state.JobUpdateRef(job.Id, event.Download.DataSourceRef); err != nil {
			return err
		}

		return state.ProjectUpdateDataRef(&pb.Ref_Project{
			Project: job.Application.Project,
		}, job.Workspace, event.Download.DataSourceRef)

	case *pb.RunnerJobStreamRequest_ConfigLoad_:
		return state.JobUpdate(job.Id, func(jobpb *pb.Job) error {
