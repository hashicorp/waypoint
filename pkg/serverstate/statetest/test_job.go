...
func TestJobCreate_singleton(t *testing.T, factory Factory, rf RestartFactory) {
	...
	t.Run("create with a dependency cycle", func(t *testing.T) {
		...
		// Create jobs
		err := s.JobCreate(ptypes2.TestJobNew(t, &pb.Job{
			Id:        "A",
			DependsOn: []string{"C"},
		}), ptypes2.TestJobNew(t, &pb.Job{
			Id:        "B",
...
func TestJobIsAssignable(t *testing.T, factory Factory, rf RestartFactory) {
	t.Run("no runners", func(t *testing.T) {
		...
		// Create a build
		result, err := s.JobIsAssignable(ctx, ptypes2.TestJobNew(t, &pb.Job{
			Id: "A",
		}))
		require.NoError(err)
	...
	t.Run("any target, runners exist", func(t *testing.T) {
		...
		// Register a runner
		require.NoError(s.RunnerCreate(ptypes2.TestRunner(t, nil)))
	...
	t.Run("any target, runners ByIdOnly", func(t *testing.T) {
		...
		// Register a runner
		require.NoError(s.RunnerCreate(ptypes2.TestRunner(t, &pb.Runner{
			ByIdOnly: true,
		})))
	...
	t.Run("ID target, match", func(t *testing.T) {
		...
		// Register a runner
		runner := ptypes2.TestRunner(t, nil)
