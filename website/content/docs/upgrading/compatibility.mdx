...
description: >-
  Waypoint version compatibility is determined based on the protocol version and
  not the release version such as "1.2.3". Two different Waypoint release
  versions are compatible so long as their protocol versions are compatible.
---
...
1. We promise that there will always be a release with overlapping protocol
   versions to allow all Waypoint components to safely upgrade.

1. For Waypoint servers, we promise that each subsequent release will be compatible with the
