...
description: |-
  Waypoint version compatibility is determined based on the protocol version and not the release version such
as "1.2.3". Two different Waypoint release versions are compatible so long as their protocol versions are compatible.
---
