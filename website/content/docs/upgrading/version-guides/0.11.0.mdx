...
There are no breaking changes in the protocol or `waypoint.hcl` configuration file. However, there was a bug related to the usage of dynamic configuration which will require attention during the upgrade. Those details are below.
...
When upgrading to Waypoint 0.11.0, follow your normal upgrade procedures. To
...
procedure. If this fails, proceed with the below remediation steps. There are
three options to upgrade to v0.11.0 if you are unable to create snapshots.
The first is, you do not need a snapshot of the database before upgrading, then
run server upgrade with the flag `-snapshot=false` to proceed with upgrading:
