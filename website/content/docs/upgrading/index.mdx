...
As of 0.2.1, Waypoint provides a command for users to automatically upgrade
...
requested from the passed in flag to the upgrade command.

~> **Note:** Due to where Waypoint server stored its database in Nomad at `/alloc`,
the automated upgrade command cannot migrate the old servers database to a new one.
This is only an issue if running Waypoint server 0.2.0 and older. Please follow
the manual upgrade guide in this case.

When the upgrade command is executed, the following takes place:

- Takes a snapshot of the current server database and save snapshot locally. The
  snapshot file can be saved elsewhere if a new name is passed in through using
  `-snapshot-name`, or optionally skipped all together
- Run the upgrade for the requested platform:
  - `docker` - This platform will force pull the image requested to update the
    container in docker.
  - `kubernetes` - This platform will use `kubectl` to set the image requested on
    the waypoint-server pod. By default, Waypoints StatefulSet is configured to
    `RollingUpdate`, so the pod will pull the requested version shortly after
    running the Waypoint upgrade command.
...
Standard Upgrade guide using the _Automated Upgrade_ Server section.
