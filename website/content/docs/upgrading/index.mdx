...
be able to take snpashots prior to upgrading your server.
...
Waypoint server in place. Simply put, for each platform, Waypoint attempts to
update the server image to latest by default, or to the server image requested
from the passed in flag to the upgrade command.
...
2. Check the help text from running `waypoint server upgrade`
