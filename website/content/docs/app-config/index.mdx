...
Configuration set via the `waypoint.hcl` file must be _synced_ to take
effect with the application. This happens automatically during any
`waypoint up` or `waypoint deploy`, but can also be triggered manually
using the `waypoint config sync` CLI command.

When configuration is synced, any changes will affect
...
This behavior is true for all methods of syncing.
