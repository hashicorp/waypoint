...
description: >-
  Waypoint can configure your application by setting environment variables and
  writing files. Both the environment variables and files can use values sourced
  from external systems such as Kubernetes ConfigMaps, HashiCorp Vault, Amazon
  Parameter Store, etc.
---
...
1. Waypoint will wait up to 30 seconds for your application to gracefully exit.

1. After 30 seconds, Waypoint will send the `SIGKILL` signal to your
