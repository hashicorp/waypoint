...
- [Consul](/plugins/consul#consul-configsourcer) - Read values from Consul key
  value storage.
...
config source to be used within a specific scope. This can be either `global`,
(the default), `project`, or `app`-scoped. For a given app or project, the config
source with the "most specific" scope will be used.
