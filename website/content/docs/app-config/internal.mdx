...
description: |-
  Internal values are static or dynamic configuration values that can be referenced by environment variable or files to compose values.
---
...
or [files](/docs/app-config/files) to compose values. An example common
usecase is building a database connection string which requires a database
host, username, password, etc. that may come from difference sources.
