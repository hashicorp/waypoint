...
but to handle more complex deployments, Waypoint needs the ability to add custom steps.
We plan to add the ability to script custom steps like container scanning or integration
with project management tools. Also, custom pipelines will allow users to compose their
own Waypoint pipelines with custom steps and embedded pipelines
