...
to the produciton workspace. We'll need to figure the Trigger to use the
...
we can run the `waypoint trigger list` command:
...
First, we can inspect the Trigger URL configuration to see how it's configured:
...
For this trigger to run in the `production` workspace, we can update the
...
you'll also now see an HTTP url:
...
Then you will be able to simply add a curl request to your CI configuration
to execute deploying the latest build with Waypoint:

```
curl -X POST -H "Authorization: OAuth $WP_ACCESS_TOKEN" https://waypoint.server.address/api/v1/trigger/fDKB5nkhl66Hknh
```
