...
a runner in your server if you are upgrading from an old server version. A fresh
installation of Waypoint server 0.3.0 will automatically include runners.
...
would if you were to start a new one. You'll want to include a runner stanza in
your `waypoint.hcl`, or enable git through the UI. Also ensure your Waypoint
...
to the project homepage after running `waypoint ui -authenticate`. From there,
you can click on the settings gear icon on the right to navigate to the project
settings. Fill in the requested fields to setup git on your project.

### Add to an existing Waypoint project

If you already have a Waypoint project, you can add git integration just as you
would if you were to start a new one. You'll want to include a runner stanza in
your `waypoint.hcl`, and ensure your Waypoint server is [at least 0.3.0 and has
runners enabled.](#Waypoint-Server-Requirement)
...
TODO - where to look in the logs when git polling fails, what to search/grep for, etc
