...
the existing version in an environment. While both versions are running, DNS is
...
or automated through scripts or some other means; but Waypoint pipelines can orchestrate
this! Waypoint pipelines can execute configured phases of your app's build, deployment,
and release lifecycle, but also run custom steps, such as modifying Consul config
