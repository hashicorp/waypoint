...
```
        registry {
          use "docker" {
            image = "registry.example.com/image" tag   = "latest"
          }
        }
```

You should configure docker authentication to push to the private registry with
something like: `echo $MY_SECRET | docker login registry.example.com -u USERNAME
--password-stdin` where `$MYSECRET` is an env var with your secret and
`USERNAME` is replaced with your actual registry username. Therefore Waypoint
does not need to know your private registry credentials since it calls the
Docker API, which should already has them by the time `waypoint build` happens.
