...
description: >-
  Labels are key/value pairs that can be specified by the user and attached to
  Waypoint operations. Labels can be used to specify any information that may be
  useful for the user, but are not directly interpreted by Waypoint itself.
---
