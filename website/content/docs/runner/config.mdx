...
These workspaces match up to your `dev` and `prod` environments in your platform.
For this example, let's assume that is Nomad (with an ACL-enabled cluster), and
your `dev` and `prod` environments are separated into different namespaces, also
named `dev` and `prod`. Accordingly, you could set your runner config to use workspace-scoped
config, where a Nomad token with permissions to submit jobs to the `dev` namespace
is used for the `dev` Waypoint workspace, and set another token with permissions to
...
only have permissions in the corresponding platform for the corresponding environment
which you configured. Operations in the `dev` workspace will not work in `prod`, and likewise
operations in the `prod` workspace will not have permissions to work in `dev`.
