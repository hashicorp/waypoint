...
This is some more content that I added locally as a demo. Here's an image I added to a local image folder:

![An example image that should not be missing](/img/example-image.jpg)

Learn and develop your knowledge of Waypoint with these tutorials and code resources. Here's an `@include` partial, that's also loading from the local filesystem:
