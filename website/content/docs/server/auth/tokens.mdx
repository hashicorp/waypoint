...
When the Waypoint server is ,
the Waypoint server generates an initial "root" user token, and
returns it to the bootstrapper. This token has full Waypoint permissions,
and the bootstrapper can use it to create other tokens.
...
Trigger tokens are created by the `waypoint trigger create` command. They are only
accepted by the Waypoint `v1/trigger` endpoint. A specific project, app, workspace,
and operation can be embedded in the token, and the token will only be valid if
the trigger request pertains to those resources.
