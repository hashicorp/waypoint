...
description: >-
  Waypoint can be configured to use OpenID Connect (OIDC) for authentication.
  This enables you to use existing identity providers such as Google, Okta,
  GitLab, and more.
---
...
1. Look for an existing user account with an email address matching the
   `email` claim. This is only done if the identity provider specifies
   `email_verified` as true.

1. Create and link a new user with a random username.
