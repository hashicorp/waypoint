...
The server which is installed by the `waypoint install` command runs a Waypoint
server in a Docker container, which is actually running the `waypoint server run`
command:
...
will also use the Waypoint Helm chart for the installation, setting sensible default
values for the chart values.
