...
~> **Internal beta:** `waypoint context verify` is not in any released
version yet. In the mean time, you can verify your connection by running
a `waypoint init` in a directory with a `waypoint.hcl` file.
