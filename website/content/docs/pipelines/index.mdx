...
Each time a pipeline is run, Waypoint keeps track of the steps and run results.
...
this pipelines "build-prod" step will run in the production workspace.

Notice that in the app definition, it contains two build stanzas, one of which
is _workspace scoped_. That means when the "build-prod" step runs, it will
use that workspace scoped build plugin pack rather than the default "docker" plugin.
