...
us know your experience with it, feature requests and enhancements, or any bugs
you might find via our [Discuss forums](https://discuss.hashicorp.com/c/waypoint/51)
or [GitHub](https://github.com/hashicorp/waypoint/issues). We look forward to
hearing about your experience and ways Waypoint can help make your experience
delivering your applications even better!

Waypoint before the introduction of Pipelines has always had a relatively fixed
built-in pipeline with the `waypoint up` CLI which ran a build, deploy, and
release. An "Up" was a special operation that encapsulated a Build, Deploy, and
...
and deploying an application to production. However, as projects and teams
...
Running tests after a build, security scanning artifacts prior to a deploy, and
so on. This was not easily possible with the current Waypoint model. Custom
Pipelines are an answer to this problem. With this tech preview, users can
define pipelines that can run in whichever order required to deliver an
application. Additionally, pipelines allow for custom execution steps through
the "Exec" plugin that will run any kind of bash script as part of the pipeline
execution for operations like running tests or security scanning.

Pipelines give users a way to tell Waypoint what order these operations should
be run in, as well as any operation options that might need to be configured
like disabling an artifact push for the build step, or pruning old deployments
on a release in the release step. Waypoint projects still expect the
application to be defined outside of a pipeline in its own `app` stanza.
...
~> **NOTE**: Currently, Waypoint only allows for 1 level of nesting when defining an embedded pipeline. However, Pipelines can also be used as a reference within a step.
...
Below is an example that redefines the Up operation to be a pipeline with 3 steps for a build, deploy, and release. This example pipeline could be used as a starting point for more complicated pipelines.
