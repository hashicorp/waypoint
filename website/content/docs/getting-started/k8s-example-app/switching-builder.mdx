...
In this sample directory we've included 2 files, a default `Dockerfile` and an `nginx/default.conf` that contains the necessary information for NGINX hosting our application. If you inspect these files,
you'll note that they are both set to run our application on port 80, which is different from the default port that the pack builder uses. As part of updating our `build` process, we also need to set the
necessary changes to the rest of the `waypoint.hcl` file to support the new build.
...
build {
      ...
      registry {
        use "docker" {
          image = "waypoint.local/sample/example-nodejs"
          tag = "latest"
        }
       }
    }
