...
We recently released a [Tech Preview of custom pipelines](https://www.waypointproject.io/docs/pipelines),
introducing the ability to script custom steps, such as running tests after a
build or security scanning artifacts prior to a deploy. As we work to bring this
feature out of Tech Preview, custom pipelines will grow to allow users to compose their
own Waypoint pipelines with custom steps, embedded pipelines, and further
customizable steps and plugins, such as integrations with project management tools.
