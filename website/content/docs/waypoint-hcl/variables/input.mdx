...
also be used to [fetch values from remote sources](/waypoint/docs/waypoint-hcl/variables/dynamic).
...
[`variable` block](/waypoint/docs/waypoint-hcl/variable). Example variable declaration
blocks include:
...
The name of a variable can be any valid [identifier](/waypoint/docs/waypoint-hcl/syntax#identifiers).
...
[type of value](/waypoint/docs/waypoint-hcl/syntax/expressions#types-and-values) that
...
from within [expressions](/waypoint/docs/waypoint-hcl/syntax/expressions) as
...
[Top-level parameters](/waypoint/docs/waypoint-hcl#top-level-parameters), such as the
...
- In the project settings via [the Waypoint server's UI](/waypoint/docs/server#waypoint-server).
...
as default values on the Waypoint [runner](/waypoint/docs/runner). For local operations
...
[literal expressions](/waypoint/docs/waypoint-hcl/syntax/expressions#literal-expressions)
...
- In the project settings via [the Waypoint server's UI](/waypoint/docs/server).
- Automatically loaded variable definition files, named with the pattern
  `*.auto.wpvars`.
