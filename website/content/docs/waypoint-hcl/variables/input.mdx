...
<!-- TODO krantzinator: get a Learn tutorial up -->

> **Hands-on:** Try the [Customize Waypoint Configuration with Input Variables]() tutorial on HashiCorp Learn.
...
When you declare variables in the waypoint.hcl, you can set their values using
CLI options, environment variables, `auto` files, and the Waypoint server UI.

## Declaring an Input Variable

Each input variable referenced in the waypoint.hcl must be declared using a
`variable` block:
...
be unique among all variables in the same waypoint.hcl. This name is used to
assign a value to the variable and to reference the variable's value when
evaluating the configuration.

<!-- TODO krantzinator -->

The name of a variable can be any valid [identifier]([docs link]#identifiers)

<!-- TODO krantzinator: any limits? -->
...
For example, given the below waypoint.hcl, you could run `waypoint build`
...
Within the waypoint.hcl that declared a variable, its value can be accessed from
within [expressions](TODO) as `var.<NAME>`, where `<NAME>` matches the label
...
so we do not recommend using Waypoint with PowerShell when you are on Windows.
...
definition in the waypoint.hcl, you will get an error.
