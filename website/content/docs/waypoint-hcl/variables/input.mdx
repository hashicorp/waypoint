...
shared with the user via these methods will generate a SHA256 encoded value using
the server cookie as the salt along with the set value. Therefore the user running an
operation can verify if the value they expected to be used was used, with the output
SHA256 encoded value.
