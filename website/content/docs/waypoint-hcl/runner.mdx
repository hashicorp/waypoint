...
as Git auth information your `waypoint.hcl` file. See
...
- `url` `(string)` - The URL for the Git repository. This can be any supported
  URL by `git clone`. For example: `https://github.com/hashicorp/waypoint-examples.git`
...
- `password` `(password: "git")` - Password for basic auth. For GitHub, this
  is typicall a [personal access token](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token).
