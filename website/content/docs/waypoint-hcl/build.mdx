...
The `build` stanza describes how an application is built. During the
build operation, Waypoint converts application source into an artifact
and optionally pushes it to a remote registry. Read more about the
[build phase of the application lifecycle here](/docs/lifecycle/build).

Exactly one `build` stanza is required within an application.
...
[registry]: /docs/waypoint-hcl/registry 'Registry Stanza'
