...
description: >-
  Waypoint configuration is usually written in [HCL
  syntax](/docs/waypoint-hcl/syntax) which is designed to be relatively easy for
  humans to read and update. Waypoint also supports JSON. This syntax is useful
  when generating configurations programmatically, since existing JSON libraries
  can be used to prepare the generated configuration files.
---
