...
separate backend and frontend processes or with a project that is a monorepo.
...
And then we can select specified `app` with paramater `-app <app-label>` . 
An example we want deploy frontend `app` on monorepo project, we can run 
`waypoint deploy -app frontend` for deploying frontend `app`.
Also can be used in `waypoint build` , `deploy` , `release` and `up`
