...
configuring the deployment of applications.
...
1. **Install and Initalize Waypoint with `waypoint/init`.** This command will
   install the waypoint CLI, start up a remote docker instance and run
   `waypoint init`.
1. **Run the build, deploy, and release.** We can leverage the orb's `up`
   command to do so
