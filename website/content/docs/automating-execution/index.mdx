...
sidebar_title: Automating Execution
description: |-
...
This section documents the currently recommended best practices when running
Waypoint in that way, and in some cases documents examples for popular
systems or providers where you may run Waypoint.

As a general rule, it is perfectly fine to simply run
`waypoint` commands in an environment that is not on a developer
workstation. For example, if you currently have a deployment tool
or script that runs on a remote machine based on a trigger, you could,
in place of that, run Waypoint, including by invoking it in
...
binary will need access to the [Waypoint server](#todo). Locally, it is recommended
that developers create [contexts](#todo) to store information about the remote
...
For a complete list of variables for configuring the client, see [todo](#todo).
...
very useful to utilize [labels](#todo). Often, these environments might provide
...
```
$ waypoint deploy -label git_sha=$GITHUB_SHA build_id=$BUILD_ID build_hostname=$(hostname)
```

## Locking

Waypoint does not currently lock workspaces, projects, or applications,
so there may be safety issues in concurrent deployments. Until these
safeguards exist in Waypoint itself, it is recommend to avoid
concurrent execution of Waypoint against the same workspace.
