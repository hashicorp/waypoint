...
1. **Install Waypoint from the official source.** If you are
...
1. **Run `waypoint init`.** This depends on the environment variables listed
   below and documented in the [Automating Execution](/docs/automating-execution#init)
   overview.
1. **Run the build, deploy, and release.** In this case, instead of using `waypoint up`,
