...
1. A line starting with # serves as a comment. Put a backslash ("")
   in front of the first hash for patterns that begin with a hash.

1. Trailing spaces are ignored unless they are quoted with backslash ("").

1. An optional prefix "!" which negates the pattern; any matching file
...
1. If the pattern ends with a slash, it is removed for the purpose of the
...
1. If the pattern does not contain a slash /, Git treats it as a shell glob
...
1. Otherwise, Git treats the pattern as a shell glob suitable for
...
1. Two consecutive asterisks ("**") in patterns matched against full
