...
- `-telemetry-opencensus-agent-addr=<string>` - Address of an OpenCensus agent or collector available to receive OpenCensus formatted telemetry, traces and stats (commonly port 55678). Example: localhost:55678
- `-telemetry-opencensus-agent-insecure` - Disables client transport security for the OpenCensus agent exporter's gRPC connection.
- `-telemetry-datadog-trace-addr=<string>` - Address of a DataDog agent available to accept traces (commonly port 8126). Example: localhost:8126
- `-telemetry-opencensus-zpages-addr=<string>` - If set, Waypoint will run a telemetry zPages server at this address. Typically this is set to something like localhost:9999, where trace debug information could be viewed at http://localhost:9999/debug/tracez, and rpc stats at http://localhost:55679/debug/rpcz. More information at https://opencensus.io/zpages/
