...
- `-app-poll` - Enable polling to continuously generate status reports for apps. This is only valid if a Git data source is supplied.
- `-app-poll-interval=<string>` - Interval between polling to generate status reports if polling is enabled.
