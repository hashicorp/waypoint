...
description: 'Create and register a trigger URL and register it to Waypoint server'
---
...
Create and register a trigger URL and register it to Waypoint server
...
- `-artifact-id=<int>` - The sequence number (short id) for the artifact to use in an operation.
- `-disable-push` - Disables pushing a build artifact to any configured registry for build operations.
- `-build-id=<int>` - The sequence number (short id) for the build to use in an operation.
- `-deployment-id=<int>` - The sequence number (short id) for the deployment to use in an operation.
- `-release-id=<int>` - The sequence number (short id) for the release to use in an operation.
