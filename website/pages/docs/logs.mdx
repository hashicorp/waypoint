...
logging solution such as Datadog. Waypoint logs provide a real-time snapshot
of logs with minimal retention. The point of Waypoint logs are to debug
emergent application behavior rather than historical behavior. For most
deployments, Waypoint will retain days of logs per application.
