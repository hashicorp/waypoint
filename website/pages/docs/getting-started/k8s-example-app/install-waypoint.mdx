...
-> Review the [prerequisites for this walkthrough](docs/getting-started/nomad-example-app/install-prereqs) before installing Waypoint. You will use the `ReactJS` application in the examples for this walkthrough.
...
Once the Waypoint binary location has been added to your path, execute the following command to install the Waypoint server on Kubernetes. Today, the binary defaults to a Kubernetes based installation.
...
When the build is completed, Waypoint sends the build artifacts, such as a Docker file, to the artifact registry of your choice. With Waypoint, multiple clients can concurrently connect to a single server to deploy and release applications.
