...
func TestJobAssign(t *testing.T) {
	...
	t.Run("component requirements", func(t *testing.T) {
		cases := []struct {
			Name   string
			Runner *pb.Runner
			Assign bool
...

			{
				...
				&pb.Runner{
					...
					Components: []*pb.Component{
						{Type: pb.Component_BUILDER, Name: "docker"},
					},
				},
				false,
			},

			{
				...
				&pb.Runner{
					...
					Components: []*pb.Component{
						{Type: pb.Component_BUILDER, Name: "docker"},
						{Type: pb.Component_REGISTRY, Name: "gcp"},
					},
				},
				true,
			},

			{
				...
				&pb.Runner{
					...
					Components: []*pb.Component{
						{Type: pb.Component_BUILDER, Name: "docker"},
						{Type: pb.Component_BUILDER, Name: "packer"},
						{Type: pb.Component_REGISTRY, Name: "gcp"},
						{Type: pb.Component_REGISTRY, Name: "aws"},
					},
				},
				true,
			},
		...
		for _, tt := range cases {
			t.Run(tt.Name, func(t *testing.T) {
				...
				job, err := s.JobAssignForRunner(ctx, tt.Runner)
				if !tt.Assign {
					require.Error(err)
					require.Equal(ctx.Err(), err)
					return
